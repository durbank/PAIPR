...
%         [r_C, c_C] = ind2sub(size(C), C_max);
    ...
    while search_R == true && length(layer_i) < 5
        ...
        dist_n = sqrt(((data_local(:,1)-row_pred)/(0.50*width_i)).^2 + ...
            0.25*(data_local(:,2)-col_pred).^2 + ((mag_local-mag_i)).^2);
...
%         col_n = col_i(end);
        ...
        try
            pp = csaps(col_i, movmedian(row_i, 4));
            row_pred = round(fnval(fnxtr(pp), col_pred));
        catch
            row_pred = round(median(row_i(end-4:end)));
        end
