...
function [accum_table] = accum_distGamma(latitude, longitude, elevation, ...
    collect_time, accum_data, accum_yr)
...
accum_table = table(repmat(latitude, size(gamma_params,1), 1), ...
    ...
    repmat(collect_time, size(gamma_params,1), 1), ...
    accum_yr, gamma_params(:,1), gamma_params(:,4), ...
    'VariableNames', {'Lat', 'Lon', 'elev', 'collect_time', ...
    'Year', 'gamma_shape', 'gamma_scale'});
% accum_table = table(repmat(latitude, size(gamma_params,1), 1), ...
%     repmat(longitude, size(gamma_params,1), 1), ...
%     repmat(elevation, size(gamma_params,1), 1),...
%     repmat(collect_time, size(gamma_params,1), 1), ...
%     accum_yr, gamma_params(:,1), gamma_params(:,4), ...
%     nanmedian(accum_data, 2), ...
%     gamma_params(:,2:3), gamma_params(:,5:6), 'VariableNames', {'Lat', ...
%     'Lon', 'elev', 'collect_time', 'Year', 'gamma_shape', 'gamma_scale', ...
