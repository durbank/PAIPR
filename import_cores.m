...
for i = 1:numel(cores.name)
    ...
    cores.(cores.name{i}) = struct('Easting', cores.Easting(i), 'Northing',...
        cores.Northing(i), 'depth', data(:,1), 'age', data(:,2), 'rho', ...
        fillmissing(data(:,3), 'pchip'), 'dD', fillmissing(data(:,4), ...
        'pchip'), 'd18O', fillmissing(data(:,5), 'pchip'));
end
