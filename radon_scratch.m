...
window_width = floor(size(radar.data_smooth, 2)/20);
...
cols = buffer(1:size(radar.data_smooth, 2), window_width, round(0.33*window_width));
start_col = [1 cols(1,2:end)];
end_col = [cols(end,1:end-1) size(radar.data_smooth, 2)];

start_row = 1:window_height:size(radar.data_smooth, 1)-window_height;
...
%         ylabel('Assigned probability')
        ...
        window_sz = size(data);
        Ymid = start_row(i) + ceil(window_sz(1)/2) - 1;
        Xmid = start_col(j) + ceil(window_sz(2)/2) - 1;
        m = tand(90+theta_max);
